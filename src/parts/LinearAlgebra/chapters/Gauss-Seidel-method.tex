\chapter{Gauss-Seidel法}
    ...
    \section{係数行列が狭義優対角ならば厳密解に収束すること}
        \begin{proof}
            ...
            $M_k \coloneq \max_{i=1,\dots,n} \abs{v_{k,i}}\;(v_{k,i}$ は $\bm{v}_k$ の第 $i$ 要素)とする。
            次の2つが同時に成り立つことが、$\bm{v}_k$ が $\bm{0}_n$ に収束するための十分条件である。
            \begin{enumerate}
                \item ある $k \in \naturalNumbers$ に対して $M_k = 0$ ならば $M_l = 0\;(l=k+1,k+2,\dots)$
                \item 適当な $0 < \alpha < 1$ が存在して $M_k > 0 \Rightarrow M_{k+1} < \alpha M_k$
            \end{enumerate}
            $L_*$ が正則であることと式 (2) より直ちに 1. が成り立つ。
            次に 2. を数学的帰納法で示す。
            $\tilde{\alpha}$ を次式で定義する。
            \[ \tilde{\alpha} \coloneq \min_{i=1,2,\dots,n} \frac{1}{\abs{a_{i,i}}} \sum_{j=1,\dots,n \wedge j\neq i} \abs{a_{i,j}} \]
            $A$ は優対角だから $0 < \tilde{\alpha} < 1$ である。
            \begin{align*}
                a_{1,1} v_{k+1, 1} &= -\sum_{j=2}^n a_{1,j}v_{k,j} \\
                \abs{a_{1,1}} \abs{v_{k+1, 1}} &= \abs{\sum_{j=2}^n a_{1,j}v_{k,j}} \leq \sum_{j=2}^n \abs{a_{1,j}}\abs{v_{k,j}} \leq M_k \sum_{j=2}^n \abs{a_{1,j}} \\
                \abs{v_{k+1, 1}} &\leq \frac{M_k}{\abs{a_{1,1}}} \sum_{j=2}^n \abs{a_{1,j}} \leq \tilde{\alpha}M_k
            \end{align*}
            $\abs{v_{k+1, j}} \leq \tilde{\alpha} M_k\;(j=1,2,\dots,l)\;(l\in\{1,2,\dots,n-1\})$ ならば $\abs{v_{k+1, l+1}} \leq \tilde{\alpha} M_k$ であることを示す。
            ...
            \begin{align*}
                ...
                \abs{a_{l+1,l+1}} \abs{v_{k+1, l+1}} &= \abs{-\sum_{j=1}^l a_{l+1,j} v_{k+1, j} - \sum_{j=l+2}^n a_{l+1,j}v_{k,j}} \leq \sum_{j=1}^l \abs{a_{l+1,j}}\abs{v_{k+1, j}} + \sum_{j=l+2}^n \abs{a_{l+1,j}}\abs{v_{k,j}} \\
                &\leq M_k \sum_{j=1,\dots,n \wedge j\neq l+1} \abs{a_{l+1,j}} \\
                \abs{v_{k+1, l+1}} &\leq \frac{M_k}{\abs{a_{l+1,l+1}}} \sum_{j=1,\dots,n \wedge j\neq l+1} \abs{a_{l+1,j}} \leq \tilde{\alpha} M_k
            \end{align*}
            以上より帰納的に $\abs{v_{k+1,j}} \leq \tilde{\alpha} M_k\;(j=1,2,\dots,n)$ が成り立つ。
